package dev.radmacher.rebootcore.configuration.editor;
...
import dev.radmacher.rebootcore.gui.SimplePagedGui;
import dev.radmacher.rebootcore.input.ChatPrompt;
import dev.radmacher.rebootcore.utils.ItemUtils;
...
public class ConfigEditorGui extends SimplePagedGui {
    ...
    protected ConfigEditorGui(Player player, JavaPlugin plugin, Gui parent, String file, MemoryConfiguration config, ConfigurationSection node) {
        ...
        this.config = config;
        this.node = node;
        this.blankItem = GuiUtils.getBorderItem(Material.LIGHT_GRAY_STAINED_GLASS_PANE);
        ...
        headerBackItem = footerBackItem = GuiUtils.getBorderItem(new ItemStack(Material.GRAY_STAINED_GLASS_PANE));
        final String path = node.getCurrentPath();
        this.setItem(4, configItem(Material.FILLED_MAP, !path.isEmpty() ? path : file , config, !path.isEmpty() ? path : null, ChatColor.BLACK.toString()));
        this.setButton(8, GuiUtils.createButtonItem(Material.OAK_DOOR, "Exit"), (event) -> event.player.closeInventory());
        ...
        for (final String sectionKey : sections) {
            setButton(index++, configItem(Material.WRITABLE_BOOK, ChatColor.YELLOW + sectionKey, node, sectionKey, "Click to open this section"),
        ...
        for (final String settingKey : settings) {
            ...
            if(val == null) continue;
                ...
                // toggle switch
                setButton(index, configItem(Material.LEVER, ChatColor.YELLOW + settingKey, node, settingKey, String.valueOf((Boolean) val), "Click to toggle this setting"),
                ...
                // number dial
                this.setButton(index, configItem(Material.CLOCK, ChatColor.YELLOW + settingKey, node, settingKey, String.valueOf((Number) val), "Click to edit this setting"),
                        (event) -> {
                            event.gui.exit();
                ...
                setButton(index, configItem(Material.STONE, ChatColor.YELLOW + settingKey, node, settingKey, val.toString(), "Click to edit this setting"),
                        (event) -> {
                            ...
                            paged.setItem(4, configItem(Material.FILLED_MAP, settingKey, node, settingKey, "Choose an item to change this value to"));
                            int i = 9;
                ...
                setButton(index, configItem(Material.STRING, ChatColor.YELLOW + settingKey, node, settingKey, val.toString(), "Click to edit this setting"),
                        (event) -> {
                            event.gui.exit();
                ...
                setButton(index, configItem(Material.WRITABLE_BOOK, ChatColor.YELLOW + settingKey, node, settingKey, String.format("(%d values)", ((List) val).size()), "Click to edit this setting"),
                        (event) -> {
    ...
    private boolean isMaterial(Object value) {
        Material m = Material.matchMaterial(value.toString());
        return m != null;
    }

    protected ItemStack configItem(Material type, String name, ConfigurationSection node, String path, String def) {
        String[] info = null;
    ...
    protected ItemStack configItem(Material type, String name, ConfigurationSection node, String path, String value, String def) {
        if(value == null) value = "";
        String[] info = null;
        if (configSection_getCommentString != null) {
            try {
