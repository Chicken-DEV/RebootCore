...
public class ConfigEditorListEditorGui extends SimplePagedGui {
    ...
    public ConfigEditorListEditorGui(ConfigEditorGui current, String key, List<String> val) {
        ...
        this.setItem(4, current.configItem(Material.FILLED_MAP, key, current.getCurrentNode(), key, null));
        ...
        this.setButton(8, GuiUtils.createButtonItem(Material.LAVA_BUCKET, ChatColor.RED + "Discard Changes"), (event) -> event.player.closeInventory());
        this.setButton(0, GuiUtils.createButtonItem(Material.REDSTONE, ChatColor.GREEN + "Save"), (event) -> {
            saveChanges = true;
            event.player.closeInventory();
        });
        this.setButton(1, GuiUtils.createButtonItem(Material.CHEST, ChatColor.BLUE + "Add Item"),
                (event) -> {
                    event.gui.exit();
    ...
    void redraw() {
        ...
        for (String item : values) {
            final int index = i - 9;
            setButton(i++, GuiUtils.createButtonItem(Material.PAPER, item, "Right-click to remove"), ClickType.RIGHT, (event) -> {
                values.remove(index);
                redraw();
            });
        }
