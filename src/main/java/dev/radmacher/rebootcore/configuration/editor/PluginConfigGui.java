...
import dev.radmacher.rebootcore.RebootPlugin;
...
public class PluginConfigGui extends SimplePagedGui {
    ...
    public PluginConfigGui(RebootPlugin plugin) {
        this(plugin, null);
    }

    public PluginConfigGui(RebootPlugin plugin, Gui parent) {
        super(parent);
        this.plugin = plugin;
    ...
    private void init() {
        this.blankItem = GuiUtils.getBorderItem(new ItemStack(Material.LIGHT_GRAY_STAINED_GLASS_PANE));
        ...
        this.setButton(8, GuiUtils.createButtonItem(new ItemStack(Material.OAK_DOOR), "Exit"), (event) -> event.player.closeInventory());

        // List out all config files that this plugin has
        int i = 9;
        for (Map.Entry<String, MemoryConfiguration> config : configs.entrySet()) {
            this.setButton(i++, GuiUtils.createButtonItem(new ItemStack(Material.BOOK), ChatColor.YELLOW + config.getKey(), "Click to edit this config"),
