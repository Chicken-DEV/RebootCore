...
import dev.radmacher.rebootcore.configuration.DataStoreObject;
import dev.radmacher.rebootcore.configuration.SimpleDataStore;
...
public class CustomizableGui extends Gui {
    ...
    public CustomizableGui setDefaultItem(ItemStack item) {
        if ((blankItem = item) != null) {
            buttons.put("__DEFAULT__", (new CustomButton("__DEFAULT__")).setIcon(item.getType()));
        }
        return this;
    }
    ...
    public CustomizableGui setItem(int defaultCell, @NotNull String key, @NotNull ItemStack item) {
        ...
        if (btn == null) {
            buttons.put(key, btn = (new CustomButton(key, defaultCell)).setIcon(item.getType()));
        } else {
            ItemStack btnItem = new ItemStack(btn.icon);
    ...
    @NotNull
    public CustomizableGui setItem(int defaultRow, int defaultCol, @NotNull String key, @NotNull Material defaultItem, @NotNull String title, @NotNull String... lore) {
        final int cell = defaultCol + defaultRow * 9;
        return setItem(cell, key, defaultItem, title, lore);
    }

    @NotNull
    public CustomizableGui setItem(int defaultCell, @NotNull String key, @NotNull Material defaultItem, @NotNull String title, @NotNull String... lore) {
        CustomButton btn = key == null ? null : buttons.get(key = key.toLowerCase());
        if (btn == null) {
    ...
    @NotNull
    public CustomizableGui updateItem(@NotNull String key, @NotNull Material itemTo, @NotNull String title, @NotNull String... lore) {
        CustomButton btn = key == null ? null : buttons.get(key.toLowerCase());
        if (btn != null) {
    ...
    @NotNull
    public CustomizableGui updateItem(@NotNull String key, @NotNull Material itemTo, @NotNull String title, @Nullable List<String> lore) {
        CustomButton btn = key == null ? null : buttons.get(key.toLowerCase());
        if (btn != null) {
            this.updateItem(btn.position, itemTo, title, lore);
        }
    ...
    public CustomizableGui setNextPage(int cell, @NotNull ItemStack item) {
        ...
        if (btn == null) {
            buttons.put("__NEXT__", btn = (new CustomButton("__NEXT__", cell)).setIcon(item.getType()));
        } else {
            ItemStack btnItem = new ItemStack(btn.icon);
            ItemMeta itemMeta = item.getItemMeta();
            ItemMeta btnItemMeta = btnItem.getItemMeta();
    ...
    public CustomizableGui setPrevPage(int cell, @NotNull ItemStack item) {
        ...
        if (btn == null) {
            buttons.put("__PREV__", btn = (new CustomButton("__PREV__", cell)).setIcon(item.getType()));
        } else {
            ItemStack btnItem = new ItemStack(btn.icon);
            ItemMeta itemMeta = item.getItemMeta();
            ItemMeta btnItemMeta = btnItem.getItemMeta();
    ...
    public static class CustomButton implements DataStoreObject<String> {
        ...
        public static CustomButton loadFromSection(ConfigurationSection sec) {
            ...
            dat.icon = sec.contains("icon") ? Material.getMaterial(sec.getString("icon")+"") : Material.STONE;
        ...
        public CustomButton setIcon(Material icon) {
            this.icon = icon != null ? icon : Material.STONE;
            _changed = true;
            return this;
        }
    }
