...
import dev.radmacher.rebootcore.gui.events.*;
import dev.radmacher.rebootcore.gui.methods.*;
...
public class Gui {
    ...
    @NotNull
    public Gui updateItem(int row, int col, @NotNull Material itemTo, @Nullable String title, @NotNull String... lore) {
        return updateItem(col + row * 9, itemTo, title, lore);
    }

    @NotNull
    public Gui updateItem(int cell, @NotNull Material itemTo, @Nullable String title, @Nullable String... lore) {
        ItemStack item = cellItems.get(cell);
        if (item != null && item.getType() != Material.AIR) {
            setItem(cell, GuiUtils.updateItem(item, itemTo, title, lore));
        }
    ...
    @NotNull
    public Gui updateItem(int row, int col, @NotNull Material itemTo, @Nullable String title, @Nullable List<String> lore) {
        return updateItem(col + row * 9, itemTo, title, lore);
    }

    @NotNull
    public Gui updateItem(int cell, @NotNull Material itemTo, @Nullable String title, @Nullable List<String> lore) {
        ItemStack item = cellItems.get(cell);
        if (item != null && item.getType() != Material.AIR) {
            setItem(cell, GuiUtils.updateItem(item, itemTo, title, lore));
        }
